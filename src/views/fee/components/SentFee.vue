<template>
    <v-container v-if="isAuthorized">
        ...
        >
            ...
            <tbody>
                <tr v-for="(fee, index) in sentFees">
                    ...
                    <td class="text-right align-top pt-2">
                        <BrkActionButton 
                            ...
                            hasDownload
                            hasCopy
                            @handleUnsendClick="handleUnsend(fee)"
                            @handleDownloadClick="handleDownload(fee)"
                            @handleCopyClick="handleCopy(fee)"
        ...
        methods:{
            ...
            async fetchFeeData(){
                ...
                try {
                    ...
                    const _sentFees = response.data.data

                    this.sentFees = _sentFees.map( fee => {
                        return {
                            ...fee,
            ...
            handleUnsend(fee){
                this.selectedFee = this.sentFees.find(_fee => _fee.TutorID === fee.TutorID)
                this.selectedFee.IsDraft = true

                this.showUnsendConfirmationDialog = true
                this.unsendConfirmationText = `Anda akan membatalkan pengiriman fee dengan nomor ${fee.FeeCode}. Fee akan kembali disimpan sebagai draft. Lanjutkan?`
            },
            handleCopy(fee){
                const urlToCopy = `https://app.barakaeducation.com/document/fee/${fee.UniquePath}`
                navigator.clipboard.writeText(urlToCopy)

                this.$root.showSnackbar(`Berhasil menyalin tautan dokumen fee ${urlToCopy}`)
            },
            handleDownload(fee){
                const routeUrl = this.$router.resolve({ path: `/document/fee/${fee.UniquePath}` }).href;
