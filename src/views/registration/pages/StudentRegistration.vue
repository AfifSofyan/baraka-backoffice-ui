<template>
    ...
    <v-container class="pa-10">
        <BrkTitle 
            title="Form Registrasi Siswa Baraka Education"
            color="primary"
            class="mb-4"
        />
...
export default{
    ...
    methods:{
        ...
        handleSubmit(){
            ...
            this.confirmationText = 'Anda Akan Mendaftarkan Siswa Baru di Baraka Education. Lanjutkan?'
        },
        handleCancel(){
        ...
        async submit(){
            this.$emit('isLoading', true)
            try {
                const data = await studentService.registerStudent(this.form);

                this.$router.push(`/registrasi/siswa/success`);
            } catch (error) {
                console.log(error)
                this.$root.showSnackbar(error.response ? error.response.data.message : error.message);
            }
