<template>
    ...
    <v-container v-if="roleName == 'superadmin'">
        <v-row class="mt-6 mb-4">
            ...
            <v-col cols="12" sm="5" md="4" class="py-0">
                <BrkButton
                    ...
                    color="primary"
                    @click="$router.push('/backoffice/laporan/akademik/create')"
                    class="mr-2"
                />
            </v-col>
        </v-row>
        ...
        <v-window v-model="tab">
            <v-window-item value="raw" class="py-8">
                <GeneratedAcademicReport 
                    ref="generated_academic_report"
                    :date-range="dateRange"
                    @isLoading="(state) => $emit('isLoading', state)"
                />
            </v-window-item>
            <v-window-item value="draft" class="py-8">
                <DraftedAcademicReport
                    ref="drafted_academic_report"
                    :date-range="dateRange"
                    @isLoading="(state) => $emit('isLoading', state)"
                />
            </v-window-item>
    ...
    <v-container v-if="roleName == 'student'">
        <v-table
            v-if="academicReportList.length > 0"
            density="compact"
            class="px-4"
        >
            <thead>
                <tr>
                    <th>No</th>
                    <th class="text-center">Periode Les</th>
                    <th class="text-center">Jumlah Kelas</th>
                    <th style="width:110px !important"></th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="(academicReport, index) in academicReportList">
                    <td class="align-top pt-2">{{ perPage * ( page - 1 ) + index + 1 }}</td>
                    <td class="align-top pt-2 text-center">{{ $root.formatDate(academicReport.StartDate) }} — {{ $root.formatDate(academicReport.EndDate) }}</td>
                    <td class="align-top pt-2 text-center">{{ academicReport.AcademicReportItems.length }}</td>
                    <td class="text-right align-top pt-2">
                        <BrkActionButton 
                            hasDetail
                            @handleDetailClick="openAcademicReportDocument(academicReport.UniquePath)"
                        />
                    </td>
                </tr>
            </tbody>
...
import GeneratedAcademicReport from '../components/GeneratedAcademicReport.vue';
import DraftedAcademicReport from '../components/DraftedAcademicReport.vue';
import SentAcademicReport from '../components/SentAcademicReport.vue';
    ...
    methods:{
        async fetchAcademicReportData(){
            ...
            try {
                ...
                const _academicReportList = response.data.data

                this.academicReportList = _academicReportList.map( academicReport => {
                    return {
                        ...academicReport,
        ...
        openAcademicReportDocument(uniquePath) {
            const routeUrl = this.$router.resolve({ path: `/document/laporan/akademik/${uniquePath}` }).href;
