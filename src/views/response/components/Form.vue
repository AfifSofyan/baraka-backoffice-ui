<template>
    <v-form v-model="validForm" @submit.prevent="handleSubmit">
        <v-row class="my-0" v-if="type == 'create'">
        ...
        <v-row>
            <v-col cols="12" md="6">
                ...
                <v-autocomplete 
                    ...
                    variant="outlined"
                    :rules="[required]"    
                    :readonly="type == 'detail' || ['tutor','student'].includes(roleName)"
                    @update:model-value="generateJoinCode"
                />
                <v-autocomplete 
                    v-model="form.ModeID"
            ...
            <v-row>
                <v-col cols="12" md="6" class="my-0">
                    <v-autocomplete 
                        ...
                        variant="outlined"
                        :rules="[required]"    
                        :readonly="type == 'detail' || roleName == 'student'"
                    />
                    <v-text-field 
                        label="Topik"
                        v-model="form.Topic"
        ...
        <v-card v-if="type=='detail'" class="my-4 pa-4">
            <p class="text-subtitle-2 mb-2">Data Log</p>
            <div class="d-flex justify-start text-body-2">
                <p>Disubmit oleh {{ form.CreatedByName }}, pada {{ $root.formatDate(form.CreatedDate) }}</p>
            </div>
            <div class="d-flex justify-start text-body-2">
                <p>Diedit oleh {{ form.UpdatedByName }}, pada {{ $root.formatDate(form.UpdatedDate) }}</p>
            </div>
        </v-card>
        ...
        <div class="d-flex justify-end" v-else>
            <BrkButton
                v-if="$root.hasAccess('Edit Responsi')"
                buttonText="Edit Data"
                color="secondary"
                @click="() => $router.push(`/backoffice/laporan/responsi/edit/${id}`)"
            />
        </div>
    ...
    <v-dialog v-model="showConfirmationDialog" width="550" persistent>
        <v-card>
            ...
            <v-card-text>
                <p class="text-justify font-16 mb-8">Anda akan {{ type == 'create' ? 'menambahkan' : 'mengedit' }} data akademik sebagaimana rangkuman di bawah ini. Centang apabila data yang dimaksud telah sesuai</p>
...
export default{
    ...
    data(){
        return{
            ...
            tutorName: JSON.parse(localStorage.getItem('name')),
    ...
    methods:{
        ...
        async loadReportDetail(){
            try {
                const data = await responseService.getReportDetail(this.id)
        ...
        async loadStudentData(){
            try{
                const result = await studentService.getStudentIDAndNameBasedOnRole()
        ...
        async loadTutorData(){
            if(this.roleName != 'superadmin'){
                if(this.roleName == 'tutor'){
                    const result = await tutorService.getTutorDetailByUserID(this.userID)
                    ...
                    this.form.TutorID = tutorData.ID
                }
        ...
        async submit(){
            ...
            try {
                ...
                if(this.type == 'create'){
                    ...
                    this.$router.push('/backoffice/laporan/responsi');
                }
                if(this.type == 'edit'){
                    const data = await responseService.updateReport(this.form, this.id);

                    this.$router.push(`/backoffice/laporan/responsi/${data.data.ReportID}`)
                }
            } catch (error) {
                console.log(error)
